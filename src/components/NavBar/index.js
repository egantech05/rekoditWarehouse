...
export default function NavBar({ onWarehousePress = () => {}, onClose = () => {}, warehouseConnected = true, onLogout = () => {} }) {

    
  const navigation = useNavigation();
  const closeThen = (fn) => () => { onClose(); fn?.(); };
  const navItems = warehouseConnected ? [
    { key: "warehouse", label: "Warehouse", icon: { name: "folder-outline", color: colors.brandHighlight }, onPress: onWarehousePress, textStyle: { color: colors.brandHighlight } },
